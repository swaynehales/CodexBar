...
struct OverviewCompactRefreshStatusTests {
    ...
    @Test func `only completed global refreshes replace the persisted record`() throws {
        ...
        #expect(try JSONDecoder().decode(OverviewCompactRefreshStatus.self, from: JSONEncoder().encode(updated)) == updated)
    }

    @Test func `refresh label includes the date only on older days and discloses failures`() {
        ...
        let older = record.label(now: date.addingTimeInterval(86400), calendar: calendar, locale: Locale(identifier: "en_US"))
        #expect(today != older)
        let failed = OverviewCompactRefreshStatus(completedAt: date, hadFailures: true)
        #expect(failed.label(now: date, calendar: calendar, locale: Locale(identifier: "en_US")).contains(L("compact_refresh_partial")))
    }
}
